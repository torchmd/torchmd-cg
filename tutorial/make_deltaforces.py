...
def make_deltaforces(coords_npz, forces_npz, delta_forces_npz, forcefield, psf, exclusions = ('bonds'),
                     device='cpu', forceterms = ['Bonds', 'Angles','RepulsionCG']):
    ...
    for co in tqdm(coords):
        ...
        fr = system.forces.detach().cpu().numpy().astype(np.float32).reshape([natoms,3])
